...
function draw() {;
  ...
  push();
  translate(width *(xStep/60) + (width/2), xStep)
  rotate(QUARTER_PI * xStep/60);
  drawLoopedShapes();
  pop();
            
  push();                 
  translate(-width * (xStep/60) + (-width * 1.5), -xStep)
  rotate(-QUARTER_PI * xStep/60);
  drawLoopedShapes();
  pop();
  updateLoopRecording();
...
function drawLoopedShapes() {
  ...
  to = color(200, 20, 20, 255);
  from = color(10, 20, 20, 255);
  ...
  xStep = xStep + (0.2 * sin(loopProgressRadians));
}
